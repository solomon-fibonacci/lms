<?php
    include_once "../scripts/types.php";
    
    // displays list of book types and if a certain book type has been selected,
    // books associated with the type are displayed on the same page
    if(!isset($content)){
        $content = "";
    }
    ...
    if(isset($bkType) && !is_null($bkType)){ // if a particular book type has been selected
        $content .= "<h4>Books of the type: {$bkType}</h4>";
        $sn = 1;
        if (isset($typeBooks)) {
